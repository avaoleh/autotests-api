...
def assert_course(actual: CourseSchema, expected: CourseSchema):
    ...
    # Сравниваем объекты пользователей
...
def assert_create_course_response(
    ...
    # Проверяем, что preview_file_id в ответе соответствует переданному значению
    assert_equal(response.course.preview_file.id, request.preview_file_id, "preview_file_id")

    # Проверяем, что created_by_user_id в ответе соответствует переданному значению
    assert_equal(str(response.course.created_by_user.id), request.created_by_user_id, "created_by_user_id")
...
class TestFiles:
    ...
    def test_get_file_with_incorrect_file_id(self, files_client: FilesClient):
        """
        Негативный тест: Получение файла с некорректным идентификатором.

        Отправляет GET-запрос к /files/{file_id} с недопустимым UUID.
        Ожидается статус-код 422 (Unprocessable Entity) и
        соответствующая ошибка валидации типа 'uuid_parsing'.
        """
        incorrect_file_id = "incorrect-file-id"

        # 1. Отправляем запрос с некорректным file_id
        response = files_client.get_file_api(incorrect_file_id)

        # 2. Проверяем, что код ответа 422
        assert_status_code(response.status_code, HTTPStatus.UNPROCESSABLE_ENTITY)

        # 3. Десериализуем тело ответа в схему валидационной ошибки
        response_data = ValidationErrorResponseSchema.model_validate_json(response.text)

        # 4. Проверяем тело ответа с помощью новой функции assert
        assert_get_file_with_incorrect_file_id_response(response_data)

        # 5. Проверяем JSON-схему ответа
        validate_json_schema(response.json(), response_data.model_json_schema())